...
- **Verification**: unit tests plus a scripted Playwright pass — paint → autosave → version bump, 1000×1000 frame timing, a forced 409 with a foreign write, and an offline-crash-restore run that killed and revived the backend mid-session.
- **What I rejected or caught**: the first version of "take theirs" wiped the undo history while the dialog itself promised the action was undoable — I made it apply the server board as a single undoable patch instead. In the offline test, `lsof -ti:8000 | xargs kill` matched the headless browser's connection to port 8000 and killed the browser instead of the server, so the test failed for a reason that had nothing to do with the code under test; the fix was killing only the listening process.
